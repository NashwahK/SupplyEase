...
const RetailerLoginCard = ({ toggleView }) => {
  ...
  const handleLogin = async (e) => {
    ...
    const Retaileremail = loginData.user.email;
    ...
    const { data: existingProfile, error: profileError } = await supabase
      ...
      .eq("email_address", Retaileremail)
      .single();
    
      const { data: custid, error: custerror } = await supabase
      .from("customer")
      .select("customer_id")
      .eq("email_address", Retaileremail)
      .single();
    ...
    if (existingProfile) {
      console.log(custid);
      sessionStorage.setItem("customer_id", JSON.stringify(custid));
...
export default RetailerLoginCard;
